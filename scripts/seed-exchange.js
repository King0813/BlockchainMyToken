...
module.exports = async function(callback) {
	...
	try {
		...
		// Fetch the depoloyed exchange

		const exchange = await Exchange.deployed()
		console.log('Exchagne fetched', exchange.address)
		...
		reault = await exchange.makeOrder(token.address, tokens(100), ETHER_ADDRESS, ether(0.1), { from: user1 })
		...
		orderId = result.logs[0].args.id 
		await exchange.fillOrder(orderId, {from: user2 })
		console.log('Filled order from 2 ${user1}')
